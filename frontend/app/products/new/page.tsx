...
export default function NewProductPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([ordData, rateData, vendData, setsData]) => {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              ...
              {setId && (
                <button type="button" onClick={() => setSetId("")} style={{ padding: "10px 14px", background: "transparent", border: "1px solid var(--border)", color: "var(--text-muted)", cursor: "pointer", fontSize: "11px", whiteSpace: "nowrap" }}>✕ Unlink</button>
              )}
            </div>
            <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
              <input value={newSetName} onChange={e => setNewSetName(e.target.value)} placeholder="Or create new set name…" style={{ ...inputStyle, flex: 1, fontSize: "12px" }} />
              <button type="button" disabled={!newSetName.trim()} onClick={async () => {
                const token = localStorage.getItem("token");
              ...
              }} style={{ padding: "10px 14px", background: "transparent", border: "1px solid var(--gold)", color: "var(--gold)", cursor: "pointer", fontSize: "11px", whiteSpace: "nowrap", opacity: newSetName.trim() ? 1 : 0.4 }}>+ Create</button>
            </div>
          </div>
