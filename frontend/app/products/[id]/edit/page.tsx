...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setDescription(product.description?.replace(/^\[.+?\]\s*/, "") || "");
      setPurity(product.purity || "");
      setGrossWeight(String(product.weight || ""));
      ...
      if (product.image_path) {
        setImagePreview(`${process.env.NEXT_PUBLIC_API_URL}/${product.image_path}`);
      }
      if (Array.isArray(product.stones)) {
        setStones(product.stones.map((s: { stone_name: string; weight: number; price_per_carat: number; total_price: number; notes: string }) => {
          stoneCounter++;
          return {
            id: stoneCounter,
            stone_name: s.stone_name || "",
            weight: String(s.weight || ""),
            price_per_carat: String(s.price_per_carat || ""),
            total_price: String(s.total_price || ""),
            notes: s.notes || "",
            manual_total: true,
          };
        }));
  ...
  const addStone = () => {
    stoneCounter++;
    setStones(prev => [...prev, { id: stoneCounter, stone_name: "", weight: "", price_per_carat: "", total_price: "", notes: "", manual_total: false }]);
  };

  const updateStone = (sid: number, field: string, value: string) => {
  ...
  const removeStone = (sid: number) => setStones(prev => prev.filter(s => s.id !== sid));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_API_URL}/products/${id}/image`, {
          method: "POST",
      ...
      setError((e as Error).message || "Could not update product");
    }
    setSaving(false);
  };
  ...
  return (
    <div style={{ maxWidth: "900px" }}>
      ...
      <div style={{ marginBottom: "36px" }}>
        ...
        <p className="label-caps" style={{ marginBottom: "8px" }}>✦ &nbsp; Edit Product</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", gap: "24px", alignItems: "start" }}>
        ...
        <div>
          <label style={{ cursor: "pointer", display: "block" }}>
            ...
            <input type="file" accept="image/*" onChange={e => { const f = e.target.files?.[0]; if (f) applyImageFile(f); }} style={{ display: "none" }} />
          </label>
          {imagePreview && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. 1 Line Ruby Emerald Necklace" style={inputStyle} />
          </div>

          <div>
            <FieldLabel>Metal Type</FieldLabel>
            <div style={{ display: "flex", gap: "8px" }}>
              {METAL_TYPES.map(m => <button key={m} onClick={() => handleMetalChange(m)} style={pillStyle(metalType === m)}>{m}</button>)}
            </div>
          </div>
          ...
          {(metalType === "Silver" || metalType === "Platinum") && (
            <div>
              ...
              <input value={purity} onChange={e => setPurity(e.target.value)} style={inputStyle} />
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: metalType === "Gold" ? "1fr 1fr 1fr" : "1fr", gap: "16px" }}>
            <div>
              ...
              <input type="number" value={grossWeight} onChange={e => setGrossWeight(e.target.value)} placeholder="e.g. 59.63" style={inputStyle} />
            </div>
            {metalType === "Gold" && (
              <>
                <div>
                  <FieldLabel>Gold Weight (g)</FieldLabel>
                  <input type="number" value={goldWeight} onChange={e => setGoldWeight(e.target.value)} placeholder="e.g. 52.00" style={inputStyle} />
                </div>
                <div>
                  <FieldLabel>Gold Rate ₹/g</FieldLabel>
                  <input type="number" value={goldRate} onChange={e => setGoldRate(e.target.value)} placeholder="e.g. 12160" style={inputStyle} />
                </div>
              </>
            )}
          </div>
          ...
          {metalType === "Gold" && goldValue > 0 && (
            <div style={{ padding: "12px 16px", background: "rgba(201,168,76,0.06)", border: "1px solid var(--border-gold)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <p style={{ fontSize: "10px", color: "var(--gold)", letterSpacing: "0.1em" }}>GOLD VALUE PREVIEW</p>
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "18px", color: "var(--gold)" }}>₹{fmt(goldValue)}</p>
            </div>
          )}

          <div>
            <FieldLabel>Description (optional)</FieldLabel>
            <input value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g. Traditional bridal necklace" style={inputStyle} />
          </div>
          ...
          <div>
            ...
            <select value={orderId} onChange={e => setOrderId(e.target.value)} style={{ ...inputStyle, cursor: "pointer" }}>
              ...
              {orders.map(o => <option key={o.id} value={o.id}>{o.notes || o.id.slice(0, 8)}</option>)}
            </select>
          </div>
      ...
      <div style={{ marginTop: "32px", border: "1px solid var(--border-gold)", overflow: "hidden" }}>
        <div style={{ background: "var(--gold-subtle)", borderBottom: "1px solid var(--border-gold)", padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p className="label-caps" style={{ fontSize: "9px" }}>Stones & Materials</p>
          <button onClick={addStone} style={{ padding: "6px 16px", background: "transparent", border: "1px solid var(--gold)", color: "var(--gold)", fontFamily: "'Didact Gothic', sans-serif", fontSize: "10px", letterSpacing: "0.08em", cursor: "pointer" }}>+ Add Stone</button>
        </div>
        ...
        {stones.length === 0 ? (
          <div style={{ padding: "20px", textAlign: "center" }}>
            <p style={{ fontFamily: "'Cormorant', serif", fontSize: "14px", fontStyle: "italic", color: "var(--text-muted)" }}>No stones — click "+ Add Stone" to add</p>
          </div>
        ) : (
          stones.map(stone => (
            <div key={stone.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1.5fr 32px", borderBottom: "1px solid var(--border)" }}>
              <div style={{ padding: "8px 10px", borderRight: "1px solid var(--border)" }}>
                <input value={stone.stone_name} onChange={e => updateStone(stone.id, "stone_name", e.target.value)} placeholder="e.g. Rubies" style={{ ...inputStyle, padding: "6px 10px", fontSize: "12px" }} />
              </div>
              <div style={{ padding: "8px 10px", borderRight: "1px solid var(--border)" }}>
                <input type="number" value={stone.weight} onChange={e => updateStone(stone.id, "weight", e.target.value)} placeholder="23.9" style={{ ...inputStyle, padding: "6px 10px", fontSize: "12px" }} />
              </div>
              <div style={{ padding: "8px 10px", borderRight: "1px solid var(--border)" }}>
                <input type="number" value={stone.price_per_carat} onChange={e => updateStone(stone.id, "price_per_carat", e.target.value)} placeholder="1800" style={{ ...inputStyle, padding: "6px 10px", fontSize: "12px" }} />
              </div>
              <div style={{ padding: "8px 10px", borderRight: "1px solid var(--border)" }}>
                <input type="number" value={stone.total_price} onChange={e => updateStone(stone.id, "total_price", e.target.value)} placeholder="auto"
                  style={{ ...inputStyle, padding: "6px 10px", fontSize: "12px", color: stone.manual_total ? "var(--gold)" : "var(--text-primary)" }} />
              </div>
              <div style={{ padding: "8px 10px", borderRight: "1px solid var(--border)" }}>
                <input value={stone.notes} onChange={e => updateStone(stone.id, "notes", e.target.value)} placeholder="optional notes" style={{ ...inputStyle, padding: "6px 10px", fontSize: "12px" }} />
              </div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                <button onClick={() => removeStone(stone.id)} style={{ background: "transparent", border: "none", color: "var(--text-muted)", cursor: "pointer", fontSize: "14px", padding: "4px" }}>✕</button>
              </div>
            </div>
          ))
        )}
      ...
      <div style={{ marginTop: "24px", border: "1px solid var(--border-gold)", overflow: "hidden" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", borderBottom: "1px solid var(--border)" }}>
          <div style={{ padding: "14px 20px", borderRight: "1px solid var(--border)" }}>
            <p style={{ fontSize: "10px", letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--text-muted)", marginBottom: "4px" }}>Studio Cost Price (optional)</p>
            <p style={{ fontSize: "10px", color: "var(--text-muted)", fontStyle: "italic" }}>What the studio paid for this product</p>
          </div>
          <div style={{ padding: "10px 16px" }}>
            <input type="number" value={costPrice} onChange={e => setCostPrice(e.target.value)} placeholder="e.g. 750000"
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", background: "var(--gold-subtle)", borderTop: "2px solid var(--border-gold)" }}>
          <div style={{ padding: "16px 20px", borderRight: "1px solid var(--border-gold)" }}>
            <p style={{ fontSize: "10px", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--gold)", marginBottom: "4px" }}>Final Price *</p>
            {manualFinalPrice ? (
              <button onClick={() => setManualFinalPrice(false)} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontSize: "10px", color: "var(--gold)", fontStyle: "italic", textDecoration: "underline", fontFamily: "'Didact Gothic', sans-serif" }}>
                ↩ Reset to auto ({autoFinal > 0 ? `₹${autoFinal.toLocaleString("en-IN", { maximumFractionDigits: 0 })}` : "—"})
              </button>
            ) : (
      ...
      {error && <p style={{ marginTop: "20px", color: "#E05C7A", fontSize: "12px" }}>{error}</p>}
